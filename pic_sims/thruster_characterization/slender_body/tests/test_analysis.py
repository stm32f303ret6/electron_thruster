...
def test_float200_values_pass_policy():
...
def test_missing_metric_is_error_exit2():
    m = _metrics()
    del m["escape_fraction_pct"]      # a REQUIRED gate's metric (f_beam is
                                      # reported-only under this policy)
...
def test_nonfinite_metric_is_error_exit2():
    verdict = lc.evaluate_gates(_metrics(escape_fraction_pct=float("nan")),
                                lc.load_policy(POLICY))
    assert verdict.status == lc.V_ERROR and verdict.exit_code == 2
