...
the 100 V low-power stage (same deck as the capstone, re-pinned here at the
new operating point)."""
...
def test_load_baseline():
    ...
    assert cfg.stage_id == "capstone.low_power"
    assert cfg.V_GAP == 100.0
    assert cfg.i_beam == pytest.approx(0.121e-3)
...
def test_dt_is_beam_cfl():
    cfg = load_config(CONFIG)
    assert cfg.dt == pytest.approx(6.927e-12, rel=1e-3)
...
def test_child_langmuir_scale():
    cfg = load_config(CONFIG)
    # 100 V over the 4.7 mm gap through the 0.5 mm spot -> ~0.083 mA scale;
    # i_beam = 1.46 * I_CL, the measured float200 emission-ceiling ratio
    assert cfg.I_CL * 1e3 == pytest.approx(0.0830, rel=5e-3)
    assert cfg.i_beam / cfg.I_CL == pytest.approx(1.46, rel=1e-2)
