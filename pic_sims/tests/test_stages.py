...
import run_ladder
from stages import STAGE_BY_ID, STAGES, Stage, topological_order
...
def test_ladder_is_acyclic_and_dependencies_exist():
    order = topological_order(list(STAGE_BY_ID))   # full registry, both groups
    seen = set()
...
def test_default_order_is_ladder_group_only():
    assert [s.id for s in topological_order()] == \
        [s.id for s in STAGES if s.group == "ladder"]


def test_characterization_spokes_depend_only_on_the_anchor():
    for stage in STAGES:
        if stage.group == "characterization":
            assert stage.requires == ("capstone.floating_body",), stage.id
...
def test_topology_detects_cycle(monkeypatch):
    ...
    monkeypatch.setattr("stages.STAGE_BY_ID", {"x.a": a, "x.b": b})
    import stages
    monkeypatch.setattr(stages, "STAGE_BY_ID", {"x.a": a, "x.b": b})
    with pytest.raises(ValueError):
        stages.topological_order(["x.a"])
