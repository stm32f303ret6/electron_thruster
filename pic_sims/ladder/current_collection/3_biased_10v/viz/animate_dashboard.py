#!/usr/bin/env python3
"""Three-panel dashboard animation for collector.biased_10v.

    Panel 1: phi(r,z) — electrostatic potential (sheath structure)
    Panel 2: n_e(r,z) — electron density
    Panel 3: I_e vs time — collected electron current vs OML/thermal theory
...
from analyze import current_history, field_rz, read_eb_scraped
...
def parse_args(argv=None):
    ap = argparse.ArgumentParser(description="collector.biased_10v dashboard")
...
def main(argv=None) -> int:
    ...
    Ie_max = max(I_e.max() * 1e6 if len(I_e) else 1,
                 cfg.I_oml_e * 1e6, cfg.I_th_e * 1e6) * 1.3
    ...
    fig.text(0.5, 0.55, "Biased Collection (+10 V)",
             fontsize=22, fontweight="bold", ha="center", va="center",
             color="#333333")
    fig.text(0.5, 0.40,
             f"Sphere probe at +{cfg.bias:.0f} V above plasma potential\n"
             "Strong electron attraction — large sheath forms around probe",
    ...
    ax3.axhline(cfg.I_oml_e * 1e6, color="tab:red", ls="--", lw=0.8,
                label=f"I_OML = {cfg.I_oml_e*1e6:.3f} μA")
    ax3.set_xlim(0, t_max_us); ax3.set_ylim(0, Ie_max)
    ax3.set_ylabel("I_e [μA]"); ax3.set_xlabel("time [μs]")
    ax3.legend(fontsize=7, loc="lower right"); ax3.grid(alpha=0.3)
    ...
    for fi in range(len(its)):
        ...
        sup.set_text(f"Biased +10 V    step {it}    t = {t_us:.2f} μs")
        writer.grab_frame()
