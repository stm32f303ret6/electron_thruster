#!/usr/bin/env python3
"""Stage-local physics and configuration for emitter.voltage_bracket.

Handles the multi-scenario source study: the live config.yaml lists all
scenarios (each carrying its own accelerating voltage -- the bracket axis),
while a run's frozen config_used.yaml carries a single resolved scenario.  ``load_config`` reads either form.  No pywarpx/PICMI, matplotlib, or
...
_SCENARIO_KEYS = ("name", "current", "hole_radius", "v_cathode")
...
class Config:
    ...
    def study_config(self) -> Optional[dict]:
        ...
        shared["electrodes"]["v_cathode"] = None   # per-scenario in this stage
        shared["scenarios"] = [
            {"name": str(s["name"]), "current": float(s["current"]),
             "hole_radius": float(s["hole_radius"]),
             **({"v_cathode": float(s["v_cathode"])} if "v_cathode" in s else {})}
...
def _build(raw: Mapping[str, Any], scenario: Optional[str],
           ...
           v_cathode_override: Optional[float] = None) -> Config:
    ...
    cfg = Config(
        ...
        v_cathode=(v_cathode_override if v_cathode_override is not None
                   else _f(ele, "v_cathode")),
        v_collector=_f(ele, "v_collector"),
...
def load_config(path: Path | str, scenario: str | None = None) -> Config:
    ...
    if "scenarios" in raw:                       # source study
        ...
        v_ov = _f(sc, "v_cathode") if "v_cathode" in sc else None
        return _build(raw, scenario, _f(sc, "current"), _f(sc, "hole_radius"),
                      tuple(table), v_cathode_override=v_ov)
