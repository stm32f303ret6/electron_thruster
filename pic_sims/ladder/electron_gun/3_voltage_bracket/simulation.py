#!/usr/bin/env python3
"""emitter.voltage_bracket: holed-anode RZ gun (WarpX PICMI).

The same holed-anode gun as emitter.holed_anode, run along the VOLTAGE axis:
each scenario carries its own accelerating voltage, bracketing the capstone
drive voltages (200 V anchor, 300 V ceiling) and the fixed-thrust throttle
command below the hardware floor (92.4 V at the ucurve_left_arm current).
...
def banner(cfg: Config, run: lc.Run) -> None:
    ...
    print("=" * 72)
    print(f"VOLTAGE-BRACKET GUN [{run.run_id}] scenario={cfg.scenario}: "
...
def parse_args(argv=None) -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="voltage-bracket gun PIC run")
