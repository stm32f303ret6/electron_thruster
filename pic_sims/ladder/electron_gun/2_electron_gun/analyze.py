#!/usr/bin/env python3
"""Cohort analysis for emitter.holed_anode (the three-scenario demonstration).
...
def write_outputs(analysis, order, cfgs, meas):
    ...
    for name in order:
        ...
        fig.suptitle(f"holed_anode [{name}]: {cfg.beam_current*1e6:.0f} uA, "
    ...
    ax.set_xticks(x)
    ax.set_xticklabels([f"{n}\n{cfgs[n].beam_current*1e6:.0f} uA, "
                        f"hole {cfgs[n].hole_radius*1e3:.1f} mm" for n in order],
                       fontsize=8)
    ax.set_ylabel("% of emitted current (steady)")
    ax.set_title("holed_anode: aperture transmission vs space charge")
...
def parse_args(argv=None):
    ap = argparse.ArgumentParser(description="holed electron gun cohort analysis")
...
def build_metrics(order, cfgs, meas):
    ...
    A, B, C = order
    metrics["collector_drop_A_to_B"] = lc.Metric.measure(
        "collector_drop_A_to_B",
        meas[A]["frac"]["collector"] - meas[B]["frac"]["collector"], "-",
        source="A collector fraction minus B collector fraction")
    metrics["anode_reduction_B_to_C"] = lc.Metric.measure(
        "anode_reduction_B_to_C",
        meas[B]["frac"]["anode"] - meas[C]["frac"]["anode"], "-",
        source="B anode fraction minus C anode fraction")
    return metrics
    ...
    try:
        ...
        if len(order) != 3:
            raise lc.ContractError(
                f"holed_anode expects exactly 3 scenarios, got {order}")
