...
every numerical choice it uses has already passed a gate somewhere cheaper.
This document checks that claim against the actual capstone deck
(the float200 baseline). First what carries over,
...
| energy conservation through a holed plate | arrival KE = drop from emission plane | both emitter steps | same bookkeeping |
| RZ ES solver + particle_shape=1 + seed 42 | Multigrid, 1e-6 | every step | identical |

## Gaps

### G1: ~~the two-node piecewise EB potential is validated nowhere~~ CLOSED 2026-08-01

Closed by the new step `capstone.two_node_laplace` (`capstone/1_two_node_laplace/`):
...
`set_potential_on_eb` rewrite. Gates:

1. assigned surface values (cathode exact, body 0.22 V cut-cell)
2. the Laplace maximum principle (exact)
3. agreement with an independent stair-step sparse-direct solve (1.86 V ≥ 20 cells out)
4. bit-exact rewrite idempotency

The cross-stage check
...
### G2: ~~the floating-potential charge pump has no analytic step~~ CLOSED 2026-08-01

Closed by the new step `collector.floating`
(`current_collection/4_floating/`): the thermal step's sphere in the same
...
which isolates the dQ accounting). Also gated:

1. equilibrium current balance
2. Gauss-law C vs 4πε₀a (measured ratio 1.07 ≈ the grounded-box correction)
3. the G5-style ledger-vs-dump consistency
...
|---|---|---|---|
| accelerating voltage | 100 V | 200 V | closed by `emitter.voltage_bracket`: beam formation gated at 200 V and 300 V (frontier commands) and at the 92.4 V over-perveance command |
...
| emission ppc | 128/cell/step | 16/cell/step | emission graininess at 16 ppc unvalidated by the emitter steps |

### G4: reservoir recycling (add_particles re-injection) is unvalidated

The collectors validate the flux reservoir. The capstone additionally recycles
...
Mitigation: report far-shell density in the capstone analysis (not gated, since the
recycle shell confounds it). The current-balance gate catches gross errors.
...
dumps after the run. No step checks the two agree.
...
### G6: beam and ambient plasma never coexist below the capstone

Emitter steps are vacuum; collector steps are beamless. Beam–plasma coupling
...
and it is why the capstone's evidence kind is system integration
regression rather than analytic verification. No cheaper step can close it.
...
prints it). The collectors showed ion-clock relaxation takes ~2 µs for
this plasma. The float200 regression numbers are therefore an 800 ns snapshot,
not a demonstrated stationary state. This ties to refactor-plan C6: a
...
1. Reduced ion mass 400 mₑ everywhere (ladder + capstone): internally
   consistent, but every real-O⁺ conclusion is an unvalidated extrapolation
   (a ladder-wide caveat, flagged in every stage README).
2. dt regime: capstone dt ≈ 5.0e-12 s (beam CFL) differs from every step's
...
3. Single grid/PPC/seed across the whole ladder. Convergence evidence is
   the refactor plan's C12 (Phase 5); the capstone inherits it.

### G11: dropped machinery (accepted trade-offs, not oversights)

1. Checkpoint/restart is not migrated (the plan defers it). An interrupted capstone
   run is FAILED and rerun (~GPU-hours at stake per interruption).
2. Probe/pinned mode, shroud, ram drift, Bz: research configs are deliberately
   ...
   2026-08-10, results in `../../../characterization/magnetized_1x/` and `magnetized_10x/`, without entering the ladder.)
...
1. G1/G2: CLOSED (2026-08-01) by the two new steps `collector.floating`
   ...
   now ladder-validated, not only lineage-validated. Both are required
