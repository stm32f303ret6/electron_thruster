...
One page answering, for each step, what it tests, what it measured, and how
close that is to theory. All numbers below come from the committed,
machine-readable `reference_results/` snapshots (each carries its run ID,
config hash, policy ID/hash, git commit, and WarpX version). This file is a
human-readable digest, never the authoritative record. Every step ran on the
committed build (WarpX 26.5, RZ electrostatic, seed 42; backend recorded per
reference) and PASSed all of its required gates.
...
The test: the same gun fires through a grounded plate with a hole.

1. A: a stiff low-current beam must pass nearly untouched.
2. B: raising the current 40× makes space charge blow the spot up against the small hole.
3. C: widening the hole must restore transmission.

This is a mechanism check. Transmission has to respond to space charge and
geometry in the right direction and magnitude.
...
identically. The 92.4 V fixed-thrust throttle command (133.5 % of the
planar scale) probes what an over-scale command costs in the isolated-gun
geometry.
...
1. The v1 policy required scenario C to lose ≥ 3 pp to space-charge current limiting.
2. The cohort refuted it. The planar I_CL over the spot is a conservative
   scale for this geometry: `holed_anode` B's loss at 79 % of the same scale
   was aperture clipping through the small hole, not virtual-cathode limiting.
3. The v1 FAIL analysis is retained under `results/`; the v2 policy gates the measured mechanism.
4. Consequence for the throttle campaign: the capstone's escape collapse at
   the same command is a can phenomenon, not gun optics.
...
capstone's configuration as well as the code.
...
I_th = n₀·e·⟨v⟩/4 · 4πa². This is exact for any convex probe, with no sheath
model and no free parameter.
...
ceiling, because an effective-potential barrier forms and the reduction grows
with a/λ_De and χ (Laframboise 1966). A value below 1 is expected physics.
...
The test: at +10 V (I_OML = 9.249 µA) the barrier deepens. The collected
fraction must fall below the 3 V value, and the sheath must grow yet stay
contained inside the domain.
...
The test: the same sphere, but nothing prescribes its voltage. The chipsat
capstone's charge-pump mechanism (Gauss-law capacitance calibration,
per-step scraped-charge accounting dQ = e·(w_i − w_e),
`set_potential_on_eb` rewrite every step) must drive it to the floating
potential, the bias where electron and ion collection balance. The anchor
brackets the two limiting ion-collection models: thermal-ion
φ_f = −0.360 V, OML-ion φ_f = −0.213 V. φ_f is independent of C, so the gate
isolates the charge accounting itself.
...
cathode rides at body − 200 V, and a 0.342 mA beam exits the lid hole into the
...
required gates PASS. F_beam 3.42 nN, escape 96.12 %, φ_body 5.40 V
(prediction ~6 V), exhaust KE 77.19 eV (predicted 77.10). At low χ the
candidate collection laws converge, so this point confirms the anchor rather
than discriminating.
...
Pre-registered hypotheses (`../characterization/slender_body/README.md`, plan section):

1. A, area-only scaling: predicted φ ≈ 4–5 V.
...
their pre-registration (`future_work/UCURVE_PLAN.md`) and reference results
intact.


## Cross-stage checks (all green in the suite verdict)

1. Collector current-fraction trend: 0.992 (0 V) ≥ 0.853 (3 V) ≥ 0.809
   (10 V). The barrier deepens with χ.
2. Sheath-radius ordering: 4.12 mm (3 V) ≤ 6.89 mm (10 V).
3. Shared plasma: all collector steps hash-match plasma/ppc/probe radius.
4. Emitter transmission consistency: no-plate 1.000 vs holed-A 0.973.
5. Capstone inherits the validated configuration: frozen plasma/dx/ppc
   hash-match `collector.thermal`'s.
6. Floating step shares thermal's configuration (new).
7. Two-node step solves the capstone's exact geometry/dx/offset (new).
...
- Stationarity (C6): steady means are windowed averages without a formal
  ...
  equilibrium on the ion clock; late dφ/dt is reported as measured.
