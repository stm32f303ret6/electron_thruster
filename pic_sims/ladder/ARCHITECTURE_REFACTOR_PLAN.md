...
>    `ladder_contract.py` (now at `pic_sims/`). It contains no physics, no
...
| `capstone.floating_body` | `capstone/2_chipsat_thruster` (was `chipsat`) | branch tips | system integration |
...
> **Layout addendum (2026-08-11).** The tree above is the layout this plan was
> written against. The structure was since split without changing the
> contract: `validation_cases/` became `pic_sims/ladder/` (rungs up to the
> anchor only), characterization spokes moved to
> `pic_sims/thruster_characterization/`, and the shared plumbing
> (`ladder_contract.py`, `stages.py` — formerly `ladder.py` — `run_ladder.py`,
> `cross_stage.py`, `tests/`) lifted to `pic_sims/`, one level up, so one copy
> serves both trees. Everything else in this document applies unchanged; stage
> ids embedded in committed manifests were frozen through the move.
