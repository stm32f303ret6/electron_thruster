#!/usr/bin/env python3
"""Stage-local physics and configuration for capstone.ucurve_valley.

Typed config + the derivations of the validated float200 capstone deck
(same geometry/plasma/numerics, driven at the 125 V fixed-thrust throttle point)
...
STAGE_ID = "capstone.ucurve_valley"
...
def load_config(path: Path | str, scenario: str | None = None) -> Config:
    ...
    if scenario is not None:
        raise ConfigError("capstone.ucurve_valley has no scenarios")
