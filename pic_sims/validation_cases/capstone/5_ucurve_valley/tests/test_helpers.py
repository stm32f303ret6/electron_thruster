...
the 125 V fixed-thrust throttle stage (same deck as the capstone, re-pinned here at the
new operating point)."""
...
def test_load_baseline():
    ...
    assert cfg.stage_id == "capstone.ucurve_valley"
    assert cfg.V_GAP == 125.0
    assert cfg.i_beam == pytest.approx(0.464e-3)
...
def test_dt_is_beam_cfl():
    cfg = load_config(CONFIG)
    assert cfg.dt == pytest.approx(6.253e-12, rel=1e-3)
...
def test_child_langmuir_scale():
    cfg = load_config(CONFIG)
    # 125 V over the 4.7 mm gap through the 0.5 mm spot -> ~0.116 mA scale;
    ...
    assert cfg.I_CL * 1e3 == pytest.approx(0.1160, rel=5e-3)
    assert cfg.i_beam / cfg.I_CL == pytest.approx(4.00, rel=1e-2)
