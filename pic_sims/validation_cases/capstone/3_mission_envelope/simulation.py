#!/usr/bin/env python3
"""capstone.mission_envelope: the chipsat at two mission operating points.

The SAME deck as capstone.floating_body -- duplicated verbatim, deliberately --
run at two `(n_e, Te, V, I)` points selected from a real year-long station-
keeping mission instead of the single point the design model's constants were
fitted at.  The claim under test is that the 0-D model in `design_sims/`
predicts what the particles actually do, far from its anchor.

A conducting can floats electrically in an ionospheric plasma; an enclosed
electron gun (cathode held a fixed supply offset below the body) fires a
prescribed beam through the lid hole.  Everything that makes this stage what it
is lives in this one file:
...
not migrated (an interrupted run is FAILED and rerun).

``--scenario`` is REQUIRED: there is no default operating point, because a run
that silently picked one would be unattributable.  Each execution creates a
fresh immutable run directory under ``outputs/`` and is COMPLETE only after
artifact and final-iteration verification.  The run's frozen config carries the
design constants and the PRE-REGISTERED predictions, so a completed run can
never be re-interpreted against a different model afterwards:
...
Run ONE WarpX case at a time on this machine.  Each scenario is ~10 h on the
14-thread CPU build (241 800 steps, 200x440, ~3 M macroparticles).
"""
...
def banner(cfg: Config, geom: Geometry, run: lc.Run) -> None:
    print("=" * 78)
    print(f"MISSION ENVELOPE [{run.run_id}] {STAGE_ID}  scenario={cfg.scenario}")
    prov = cfg.provenance
    print(f"  mission row: {prov.get('timestamp_utc')} (row {prov.get('row_index')}"
          f" of {prov.get('orbit_case')})")
    print(f"  demand: drag {cfg.drag_target_N*1e9:.3f} nN")
    pred = cfg.predicted
    print(f"  PRE-REGISTERED PREDICTION: phi_body {float(pred['phi_body_V']):+.3f} V, "
          f"F_beam {float(pred['f_beam_nN']):.3f} nN, "
          f"exhaust {float(pred['exhaust_ke_eV']):.2f} eV "
          f"(binding constraint: {pred['binding']})")
    print(f"    from laws @ {cfg.law_anchor['laws_sha256'][:12]} "
          f"anchored on {cfg.law_anchor['anchored_to']}")
    print(f"    settle time tau = {cfg.predicted_settle_time_s()*1e9:.1f} ns; "
          f"t_end is {cfg.t_end/cfg.predicted_settle_time_s():.1f} tau")
    print(f"  plasma: ne={cfg.n0:.3e} m^-3  kTe={cfg.kTe_eV*1e3:.1f} meV  "
          f"mi={cfg.ion_mass_me:.0f} me  lambda_D={cfg.lamD*1e3:.3f} mm  "
          f"(r_p/lambda_D={cfg.r_probe/cfg.lamD:.2f}, "
          f"rmax/lambda_D={cfg.rmax/cfg.lamD:.1f})  "
...
def parse_args(argv=None) -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="chipsat mission-envelope PIC run")
    ap.add_argument("--config", type=Path, default=DEFAULT_CONFIG)
    ap.add_argument("--scenario", required=True,
                    help="which mission operating point to run (REQUIRED)")
    ap.add_argument("--t-end", type=float, default=None,
                    help="override run.t_end [s] for a SMOKE run. A run made "
                         "this way is declared calibration, not evidence: the "
                         "config hash changes, so it can never join a cohort "
                         "with the real runs.")
...
def main(argv=None) -> int:
    ...
    if args.t_end is not None:
        # dataclasses.replace keeps every invariant: validate() runs again, and
        # the shortened t_end changes the config hash, so the smoke run is
        # structurally incapable of being analyzed alongside a real one.
        cfg = replace(cfg, t_end=args.t_end)
        cfg.validate_smoke()
        print(f"[SMOKE] t_end overridden to {args.t_end*1e9:.0f} ns -- this run is "
              f"CALIBRATION, not evidence; its float will not have settled.",
              flush=True)
    geom = cfg.geometry()
