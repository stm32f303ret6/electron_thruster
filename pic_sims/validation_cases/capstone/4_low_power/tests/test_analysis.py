...
def _metrics(**over):
    # Plausible 100 V / 0.121 mA values: ~3.4 nN predicted by the I*sqrt(V-phi)
    # scaling of the float200 measurement, float near +6 V.
    base = dict(escape_fraction_pct=98.0, f_beam_nN=3.4, phi_body_V=6.0,
...
def test_predicted_low_power_values_pass_policy():
...
def test_missed_mission_thrust_is_a_finding_not_a_failure():
    # The covers_600km_mean_drag gate is required: false -- 1.5 nN misses the
    # 2.0 nN mission bar but the run is still valid evidence.
    verdict = lc.evaluate_gates(_metrics(f_beam_nN=1.5), lc.load_policy(POLICY))
    assert verdict.status == lc.V_PASS, verdict.detail
    gate = next(g for g in verdict.gates if g.id == "covers_600km_mean_drag")
