# Migration plan: float200 capstone → `capstone.floating_body`

This documents how the validated **float200 chipsat capstone** from a prior
study is migrated into this repo as the top step of the validation ladder,
following the stage contract of `../../ARCHITECTURE_REFACTOR_PLAN.md`.
Companion document: `VALIDATION_GAPS.md` — what the lower steps do and do
**not** validate about this stage.
...
| Original source | capstone/2_chipsat_thruster/ stage file | Notes |
|---|---|---|
...
- **Shroud, `plasma.drift_z`, `fields.Bz_T`** (gap-LX/D experiments): baseline
  ...
  the original repo. Research sweeps stay in the original study.
...
1. Stage unit tests (no WarpX): config parsing/rejection, geometry invariants
   ...
   derived quantities vs the original study's `run_meta.json` values
