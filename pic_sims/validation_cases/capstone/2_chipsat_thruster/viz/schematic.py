#!/usr/bin/env python3
"""B&W isometric cutaway of the chipsat thruster can (capstone.floating_body).

CAD-style drawing: conducting can with lid hole, cathode disk, beam arrow,
dimension lines, and electrical labels.
...
def draw_can_cutaway(ax, geo):
    ...
    geo is a dict with keys from config.yaml geometry section.
    """
    rp = geo["r_probe"] * S
    ...
    # Wall cross-section (right and left side)
    ...
    # Lid cross-section (washer from r_slit to ri on each side)
    for sign in (1.0, -1.0):
...
def draw(geo, elec, beam, dpi):
    ...
    fig.text(0.5, 0.97, "Chipsat Thruster (200 V baseline)",
    ...
    out = CASE_DIR / "viz" / "schematic_2_chipsat_thruster.png"
