#!/usr/bin/env python3
"""B&W isometric cutaway of the chipsat can (capstone.two_node_laplace).

CAD-style drawing: conducting can with lid hole, cathode disk, NO beam,
dimension lines, and fixed-voltage labels.  Vacuum Laplace solve.
...
def draw(geo, elec, dpi):
    ...
    phi_body = elec["phi_body"]
    cathode_offset = elec["cathode_offset"]
    v_cathode = phi_body + cathode_offset
    ...
    leader(ax, (uw, vw), f"BODY = {phi_body:+.0f} V",
           (2.5, -2.0), fontsize=12, fontweight="bold")

    umid, vmid = iso((zb + zt) / 2, 0, 0)
    leader(ax, (umid, vmid), "Vacuum Laplace\n(no plasma, no beam)",
           (3.5, 2.5), fontsize=11, fontstyle="italic")
    ...
    fig.text(0.5, 0.97, "Two-Node Laplace",
             fontsize=18, fontweight="bold", ha="center", va="top",
             color="black")
    fig.text(0.5, 0.93,
             "Vacuum electrostatic solve with pinned conductor potentials",
             fontsize=12, ha="center", va="top", color="0.4")

    out = CASE_DIR / "viz" / "schematic_1_two_node_laplace.png"
...
def main(argv=None) -> int:
    ...
    draw(raw["geometry"], raw["electrical"], args.dpi)
    return 0
