...
def _metrics(**over):
    # Plausible 300 V / 0.63 mA values: ~30 nN predicted by the I*sqrt(V-phi)
    # scaling of the float200 measurement, float near +31 V.
    base = dict(escape_fraction_pct=98.0, f_beam_nN=30.0, phi_body_V=31.0,
...
def test_predicted_high_thrust_values_pass_policy():
...
def test_missed_mission_thrust_is_a_finding_not_a_failure():
    # The covers_500km_worst_drag gate is required: false -- 25 nN misses the
    # 28.4 nN mission bar but the run is still valid evidence.
    verdict = lc.evaluate_gates(_metrics(f_beam_nN=25.0), lc.load_policy(POLICY))
    assert verdict.status == lc.V_PASS, verdict.detail
    gate = next(g for g in verdict.gates if g.id == "covers_500km_worst_drag")
