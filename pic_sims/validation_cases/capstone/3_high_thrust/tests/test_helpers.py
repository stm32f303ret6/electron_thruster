...
the 300 V high-thrust stage (same deck as the capstone, re-pinned here at the
new operating point)."""
...
def test_load_baseline():
    ...
    assert cfg.stage_id == "capstone.high_thrust"
    assert cfg.V_GAP == 300.0
    assert cfg.i_beam == pytest.approx(0.63e-3)
...
def test_dt_is_beam_cfl():
    cfg = load_config(CONFIG)
    assert cfg.dt == pytest.approx(4.152e-12, rel=1e-3)
...
def test_child_langmuir_scale():
    cfg = load_config(CONFIG)
    # 300 V over the 4.7 mm gap through the 0.5 mm spot -> ~0.431 mA scale;
    # i_beam = 1.46 * I_CL, the measured float200 emission-ceiling ratio
    assert cfg.I_CL * 1e3 == pytest.approx(0.4312, rel=5e-3)
