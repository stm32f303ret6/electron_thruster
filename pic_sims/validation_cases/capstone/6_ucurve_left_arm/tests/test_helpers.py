...
the 92.4 V fixed-thrust throttle stage (same deck as the capstone, re-pinned here at the
new operating point)."""
...
def test_load_baseline():
    ...
    assert cfg.stage_id == "capstone.ucurve_left_arm"
    assert cfg.V_GAP == 92.4
    assert cfg.i_beam == pytest.approx(0.601e-3)
...
def test_dt_is_beam_cfl():
    cfg = load_config(CONFIG)
    assert cfg.dt == pytest.approx(7.181e-12, rel=1e-3)
...
def test_child_langmuir_scale():
    cfg = load_config(CONFIG)
    # 92.4 V over the 4.7 mm gap through the 0.5 mm spot -> ~0.074 mA scale;
    ...
    assert cfg.I_CL * 1e3 == pytest.approx(0.0737, rel=5e-3)
    assert cfg.i_beam / cfg.I_CL == pytest.approx(8.15, rel=1e-2)
