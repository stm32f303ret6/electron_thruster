...
def _metrics(**over):
    # Plausible H1 values at 92.4 V / 0.601 mA: delivered thrust on the 13.65 nN
    # demand, float near +32.5 V (model/ucurve_targeting.py, pre-registered
    # in ../UCURVE_PLAN.md).
    base = dict(escape_fraction_pct=96.5, f_beam_nN=13.65, phi_body_V=32.5,
...
def test_broken_current_balance_fails():
    # Steady current balance stays REQUIRED at this operating point: both
    # hypotheses predict a converged equilibrium here.
...
def test_missing_required_metric_is_error_exit2():
    # current_balance backs a REQUIRED gate; its absence is ERROR, not SKIP.
    m = _metrics()
    del m["current_balance"]
...
def test_nonfinite_metric_is_error_exit2():
    verdict = lc.evaluate_gates(_metrics(edge_phi_max_V=float("nan")),
                                lc.load_policy(POLICY))
    assert verdict.status == lc.V_ERROR and verdict.exit_code == 2
