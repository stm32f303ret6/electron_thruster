# validation_cases — verification ladder to the chipsat thruster
...
- `capstone.high_thrust` (300 V, 30.13 nN) and `capstone.low_power` (100 V, 3.42 nN) complete the three-point P-F frontier across the full hardware voltage range, bracketing the validated 200 V anchor
...
2. **Shared plumbing** (run IDs, manifests, immutable directories, gate evaluation) lives in `ladder_contract.py`. No physics, no pywarpx, no plotting.
