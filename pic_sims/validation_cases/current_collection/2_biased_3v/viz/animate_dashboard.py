#!/usr/bin/env python3
"""Three-panel dashboard animation for collector.biased_3v.
...
def parse_args(argv=None):
    ap = argparse.ArgumentParser(description="collector.biased_3v dashboard")
...
def main(argv=None) -> int:
    ...
    fig.text(0.5, 0.55, "Biased Collection (+3 V)",
             fontsize=22, fontweight="bold", ha="center", va="center",
             color="#333333")
    fig.text(0.5, 0.40,
             f"Sphere probe at +{cfg.bias:.0f} V above plasma potential\n"
             "Moderate electron attraction — sheath forms around probe",
    ...
    for fi in range(len(its)):
        ...
        sup.set_text(f"Biased +3 V    step {it}    t = {t_us:.2f} μs")
        writer.grab_frame()
