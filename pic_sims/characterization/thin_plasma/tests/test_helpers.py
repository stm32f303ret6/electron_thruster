...
def test_load_baseline():
    ...
    assert cfg.stage_id == "characterization.thin_plasma"
...
def test_grid_snaps_to_contactor_values():
    cfg = load_config(CONFIG)
    assert (cfg.nr, cfg.nz) == (272, 440)
    assert cfg.rmax == pytest.approx(0.0408)      # 30 -> 40 mm containment
    assert cfg.zmin == pytest.approx(-0.030)      # (snapped up by the grid)
...
def test_reservoir_shell():
    cfg = load_config(CONFIG)
    assert cfg.R_res == pytest.approx(0.03006)   # scales with rmax


def test_plasma_matches_collector_row():
    cfg = load_config(CONFIG)
    # n0/3 of the collector row: lambda_D grows sqrt(3)x
    assert cfg.lamD * 1e3 == pytest.approx(3.403, abs=0.01)
    assert cfg.n0 == pytest.approx(5.4233e11)
