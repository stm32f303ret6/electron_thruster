...
def test_load_baseline():
    ...
    assert cfg.stage_id == "characterization.magnetized_10x"
    assert cfg.V_GAP == 200.0
...
def test_dt_is_beam_cfl():
    cfg = load_config(CONFIG)
    assert cfg.dt == pytest.approx(5.026e-12, rel=1e-3)
...
def test_child_langmuir_scale():
    cfg = load_config(CONFIG)
    # 200 V over the 4.7 mm gap through the 0.5 mm spot -> ~0.235 mA scale
    assert cfg.I_CL * 1e3 == pytest.approx(0.2347, rel=5e-3)
...
def test_bz_is_the_single_axis_of_this_deck(tmp_path):
    """This deck is the anchor plus exactly one key; stripping it must recover
    the unmagnetized baseline (no stray key in the freeze)."""
    base = load_config(CONFIG)
    assert base.Bz_T == pytest.approx(3.0e-4)
    assert base.effective_config()["plasma"]["Bz_T"] == pytest.approx(3.0e-4)
    raw = _raw()
    del raw["plasma"]["Bz_T"]
    unmag = load_config(_write(tmp_path, raw))
    assert unmag.Bz_T is None
    assert "Bz_T" not in unmag.effective_config()["plasma"]
