#!/usr/bin/env bash
# The WIDE/LONG 10x follow-up (config_wide10x.yaml), strictly sequential:
#   b0_control (wide) -> transverse_10x (wide) -> cohort analysis under
#   acceptance_wide10x.yaml.  Campaign-level resume as campaign.sh: a
#   scenario with a COMPLETE run under THIS variant's study hash is skipped.
#
#   setsid nohup bash campaign_wide.sh > logs/campaign_wide_$(date -u +%Y%m%dT%H%M%SZ).log 2>&1 < /dev/null &
set -euo pipefail
HERE="$(cd "$(dirname "$0")" && pwd)"
PIC="$(cd "$HERE/../.." && pwd)"
CFG="$HERE/config_wide10x.yaml"
POLICY="$HERE/acceptance_wide10x.yaml"
...
check_disk() {
    ...
    if (( free < MIN_FREE_GB )); then
        echo "[wide $(stamp)] only ${free} GB free (< ${MIN_FREE_GB} GB): refusing to launch $1" >&2
        exit 3
    fi
}
...
run_scenario() {
    ...
    if [[ -n "$existing" ]]; then
        echo "[wide $(stamp)] $scn: COMPLETE run exists, skipping ($existing)" >&2
...
t10=$(run_scenario transverse_10x)
log="$HERE/logs/wide_analysis_$(stamp).log"
echo "[wide $(stamp)] analyzing -> $log" >&2
