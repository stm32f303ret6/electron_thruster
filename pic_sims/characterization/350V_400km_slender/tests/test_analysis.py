...
def _metrics(**over):
    # Plausible slender 350 V / 0.793 mA values: ~42-43 nN predicted by the
    # composed laws, float in the ~11-17 V slender-collection band.
    base = dict(escape_fraction_pct=98.0, f_beam_nN=42.6, phi_body_V=13.0,
...
def test_predicted_350V_400km_slender_values_pass_policy():
    verdict = lc.evaluate_gates(_metrics(), lc.load_policy(POLICY))
    assert verdict.status == lc.V_PASS, verdict.detail


def test_missed_mission_thrust_is_a_finding_not_a_failure():
    # The covers_400km_mean_drag gate is required: false -- 30 nN misses the
    # 32.9 nN mission bar but the run is still valid evidence.
    verdict = lc.evaluate_gates(_metrics(f_beam_nN=30.0), lc.load_policy(POLICY))
    assert verdict.status == lc.V_PASS, verdict.detail
    gate = next(g for g in verdict.gates if g.id == "covers_400km_mean_drag")
    assert gate.status == lc.GATE_FAIL and not gate.required


def test_high_float_fails_benign_gate():
    verdict = lc.evaluate_gates(_metrics(phi_body_V=75.0), lc.load_policy(POLICY))
    assert verdict.status == lc.V_FAIL


def test_choked_low_escape_fails():
...
def test_missing_metric_is_error_exit2():
    # escape_fraction_pct backs a REQUIRED gate; f_beam_nN only backs the
    # optional mission gate, whose absence is SKIP, not ERROR.
    m = _metrics()
    del m["escape_fraction_pct"]
    verdict = lc.evaluate_gates(m, lc.load_policy(POLICY))
    assert verdict.status == lc.V_ERROR and verdict.exit_code == 2


def test_missing_optional_metric_is_skip_not_error():
    m = _metrics()
    del m["f_beam_nN"]
    verdict = lc.evaluate_gates(m, lc.load_policy(POLICY))
...
def test_nonfinite_metric_is_error_exit2():
    verdict = lc.evaluate_gates(_metrics(phi_body_V=float("nan")),
                                lc.load_policy(POLICY))
    assert verdict.status == lc.V_ERROR and verdict.exit_code == 2
