#!/usr/bin/env python3
"""Three-panel dashboard animation for capstone.ucurve_valley (125 V fixed-thrust).
...
def parse_args(argv=None):
    ap = argparse.ArgumentParser(description="capstone.ucurve_valley dashboard")
...
def main(argv=None) -> int:
    ...
    fig.text(0.5, 0.55, "U-curve valley (125 V)",
             fontsize=22, fontweight="bold", ha="center", va="center",
             color="#333333")
    fig.text(0.5, 0.40,
             "Same geometry, fixed-thrust demand at 125 V\n"
    ...
    for fi in range(len(its)):
        ...
        sup.set_text(f"U-curve 125 V    step {it}    t = {t_ns:.1f} ns")
        writer.grab_frame()
