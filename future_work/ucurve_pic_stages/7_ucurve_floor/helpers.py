#!/usr/bin/env python3
"""Stage-local physics and configuration for capstone.ucurve_floor.

Typed config + the derivations of the validated float200 capstone deck
(same geometry/plasma/numerics, driven at the 78 V fixed-thrust throttle point)
...
def load_config(path: Path | str, scenario: str | None = None) -> Config:
    ...
    if scenario is not None:
        raise ConfigError("capstone.ucurve_floor has no scenarios")
