#!/usr/bin/env python3
"""Three-panel dashboard animation for capstone.ucurve_left_arm (92.4 V fixed-thrust).
...
sys.path.insert(0, str(CASE_DIR.parents[2] / "pic_sims"))  # shared plumbing (ladder_contract)
...
def parse_args(argv=None):
    ap = argparse.ArgumentParser(description="capstone.ucurve_left_arm dashboard")
...
def main(argv=None) -> int:
    ...
    fig.text(0.5, 0.55, "U-curve left arm (92.4 V)",
             fontsize=22, fontweight="bold", ha="center", va="center",
             color="#333333")
    fig.text(0.5, 0.40,
             "Same geometry, fixed-thrust demand at 92.4 V\n"
    ...
    for fi in range(len(its)):
        ...
        sup.set_text(f"U-curve 92.4 V    step {it}    t = {t_ns:.1f} ns")
        writer.grab_frame()
