...
inside = [100.0 - S[m]["V_over_tested_pct"] for m in order]   # points closed at <= 350 V
...
for xi, d, f in zip(x, duty, inside):
    ax[0].text(xi, d + 3, f"{f:.0f}%\nat \u2264350 V", ha="center", fontsize=7)
ax[0].set_xticks(x, labels)
ax[0].set_ylabel("duty cycle needed at 350 V [%]")
ax[0].set_xlabel("altitude [km] / pose")
ax[0].set_ylim(0, 165)

ax[1].bar(x, pmean, 0.6, color="tab:blue")
ax[1].axhline(30, c="k", lw=0.8, ls="--")
ax[1].text(1.6, 33, "example supply: body-mounted solar (~30 mW)", fontsize=7)
ax[1].set_xticks(x, labels)
ax[1].set_ylabel("mean beam power, float included [mW]")
